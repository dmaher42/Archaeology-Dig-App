...
SOURCES = {
    "idle": SOURCE_DIR / "asha-v5-idle-source.png",
    "run": SOURCE_DIR / "asha-v5-run-source.png",
    "jump": SOURCE_DIR / "asha-v5-jump-source.png",
    "climb": SOURCE_DIR / "asha-v5-climb-source.png",
    "interact": SOURCE_DIR / "asha-v5-interact-source.png",
    "hurt": SOURCE_DIR / "asha-v5-damage-source.png",
    "attack_pick_swing": SOURCE_DIR / "asha-v5-attack-source.png",
    "portrait": SOURCE_DIR / "asha-v5-portrait-source.png",
}
...
ROW_SOURCE = {
    ...
    "hurt": ("hurt", [0, 1, 2, 3, 4]),
    "interact": ("interact", [0, 1, 2, 3, 4, 5]),
    "climb": ("climb", [0, 1, 2, 3, 4, 5, 6, 7]),
    "push_pull": ("interact", [1, 2, 3, 4, 4, 3, 2, 1]),
}
...
def keep_visible_components(image: Image.Image, *, min_pixels: int = 120) -> Image.Image:
    rgba = image.convert("RGBA")
    ...
    selected = [
        ...
        or (
            ...
            and width * 0.10 <= (min_x + max_x) / 2 <= width * 0.90
        )
    ]
...
def extract_frame(source: Image.Image, *, frame_count: int, frame_index: int) -> Image.Image:
    cell_w = source.width / frame_count
    left = max(0, round(frame_index * cell_w))
    right = min(source.width, round((frame_index + 1) * cell_w))
    return keep_visible_components(source.crop((left, 0, right, source.height)))
...
def main() -> None:
    ...
    cleaned_sources = {
        name: improve_gameplay_readability(remove_checkerboard_background(Image.open(path)))
    ...
    metadata["source"] = "asha-v5-user-provided-sprite-sheets-2026-05-23"
    metadata["status"] = "production-candidate-asha-v5-alternative"
    metadata["productionReference"] = TARGET_REFERENCE.name
    metadata["description"] = (
        "Asha V5 alternative runtime atlas packed from user-provided idle, run, jump, "
        "damage, attack, interact, climb, and portrait strips. It is 10 percent larger "
        "than the standard Asha draw height and has a non-generative brightness, contrast, "
        "saturation, and sharpness pass for gameplay readability. It preserves the existing "
        "Journey hero-atlas contract and is available as a separate character-loader option."
    )
