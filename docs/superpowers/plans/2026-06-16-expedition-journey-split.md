...
Progress note: remaining exterior renderer-only callbacks (`drawWorldContinuityLandmark`, `drawStageEntranceFeature`, `drawDynamicEnvironmentEvent`, `drawEnvironmentInteraction`, `drawRouteGate`, `drawHazard`, `drawDiscoveryEntrance`, and `drawPremiumEgyptianChamberDoor`) have been moved into `useJourneyRenderer.js`; `ExpeditionJourney.jsx` remains the caller/orchestrator. Remaining local draw callbacks are shared asset wrappers or full interior room renderers and should move in a later scene/module split rather than as tiny renderer-only slices.
Progress note: interior room scene renderers (`drawTempleThresholdHallInterior`, `drawMummificationChamberInterior`, `drawForgottenMuralChamberInterior`, and `drawScribeLockedChamberInterior`) have been moved into the focused `journeyInteriorRenderers.js` module; `ExpeditionJourney.jsx` remains the caller/orchestrator and passes the existing room refs, constants, helpers, and scene predicates through dependencies. Source-contract tests now read the focused scene renderer module for interior drawing details.
Progress note: dev placement editor shortcut logic has been moved into the focused `useJourneyPlacementEditorShortcuts.js` hook; `ExpeditionJourney.jsx` keeps editor state, pointer drag/edit handling, and write-to-source/export ownership for later editor-logic slices. Source-contract tests now read the focused editor shortcut module for Shift+E, palette/grid, undo/redo, save, and selected-prop shortcut details.
Progress note: dev placement editor pointer/context-menu logic has been moved into the focused `useJourneyPlacementEditorPointerHandlers.js` hook; `ExpeditionJourney.jsx` keeps editor state, export/write-source ownership, and the normal gameplay keyboard loop. Source-contract tests now read the focused editor pointer module for click selection, drag movement, hover stack picking, and context-menu suppression.
Progress note: Arrival Threshold echo-trial renderer (`drawArrivalThresholdTrial`) has been moved into `useJourneyRenderer.js`; `ExpeditionJourney.jsx` still owns trial state/progression and only calls the renderer hook. This keeps the shortcut/trial behavior unchanged while removing one more visual-only callback from the main Journey component.
Progress note: exterior structure scene renderers (`drawOpeningPyramidMasonryBack`, Lost Bridge ravine/structure visuals, and Mummification/Mural/Scribe exterior doorway structures) have been moved into the focused `journeyExteriorStructureRenderers.js` module; `ExpeditionJourney.jsx` remains the caller/orchestrator and still owns gameplay state, collision, room entry, and progression. Source-contract tests now read the exterior renderer module for these visual guardrails.
